...
def chat_with_agentic_vitals(message, history):
    ...
    # Direct keyword trigger for vitals
    vitals_keywords = ['collect my vitals', 'check my vitals', 'scan my health', 'measure my heart rate', 
                       'check my stress', 'how am i doing', 'vitals now', '🎥']
    
    if any(keyword in message.lower() for keyword in vitals_keywords):
        # Directly trigger vitals collection
        yield "🤖 **I'll collect your vitals now!**\n\nStarting vitals collection..."
        time.sleep(1)
